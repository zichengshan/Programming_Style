...
ns_words = np.char.upper(ns_words)

# Do the translation
# Reference: https://www.runoob.com/python3/python3-string-maketrans.html
ns_words_leet = np.char.translate(ns_words, str.maketrans("AEIOU", "∆£!Θ_"))

# Get 2-grams
ns_words_repeat = np.repeat(ns_words_leet, 2)
ns_words_2_grams = np.reshape(ns_words_repeat[1:-1], (-1, 2))

# Count the frequency
uniq, counts = np.unique(ns_words_2_grams, axis=0, return_counts=True)
